...
- **Tests**: 19 regression tests added: chunk-edge coverage at 1-byte, 3-byte and delimiter-length chunk sizes, a `tracemalloc` ceiling asserting peak memory stays under 1MB while 16MB streams through, spill-to-disk and stay-in-memory thresholds, error draining, two tests driving a real uvicorn server over a raw socket (a 20MB upload and a refused form that must answer instead of resetting), and six covering header replace semantics: case-insensitive replacement, the `Set-Cookie` exception, deletion with `None`, comma-joined list values and the `res.file()` double-header case.
