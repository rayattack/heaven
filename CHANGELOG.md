...
Everything since 1.4.2 in one release: streaming form parsing, replace-on-assign header semantics, and a correctness and hardening pass. Six entries change observable behaviour; they are the breaking changes listed first, read them before upgrading.

- **Breaking**: **Setting A Header Replaces It Instead Of Appending**. `res.headers = key, value` and `res.header()` appended unconditionally, so assigning `Content-Type` twice sent it twice. HTTP allows a field name to repeat on the wire only when its value is defined as a comma-separated list, or for `Set-Cookie`; a duplicated singleton like `Content-Type` is protocol-invalid and clients resolve the conflict unpredictably, some taking the first line and some the last. Heaven could produce the conflict by itself: `res.file()` sets `Content-Type` and `Content-Disposition`, so a handler that had set either sent both lines. Assignment now replaces any value the response already carries for that name, matched case-insensitively, so the last write wins and built-ins override instead of duplicating. `Set-Cookie` still accumulates one line per cookie, which is how cookies are required to travel, so `res.cookie()` is unchanged. Assigning `None` now removes a header, so headers can be read (`res.headers`), replaced and deleted. Multi-valued headers keep the existing list form, `res.headers = 'Vary', ['Origin', 'Accept']`, which comma-joins on one line; code that relied on repeated assignment to stack `Link` or `Vary` lines must switch to it, the wire meaning being identical.
- **Breaking**: **A Multipart Body Without Its Closing Boundary Raises**. The old `email.parser` path silently accepted a truncated multipart body and returned whatever parts happened to be complete. A body that ends before the closing `--boundary--` now raises `ValueError` on both paths, because a truncated upload silently delivered as a shorter file is data loss. Every browser and HTTP client sends the closing boundary; only truncated or hand-built bodies are affected. An empty body with a multipart content type still parses as an empty form.
...
- **Feature**: **Class Handlers With `'module.Class#method'`**. String discovery now resolves a method as well as a function: `app.GET('/orders', 'controllers.orders.Orders#index')` registers `index` on a class that subclasses the new `heaven.Handler` export. The three objects a function handler is passed arrive as `self.req`, `self.res` and `self.ctx`, so the two styles are one contract written two ways, and hooks, streaming, schemas and subdomains behave identically for both. Methods may be sync or async. Subclassing is what types them: `Handler.__init__` carries the annotations, so a subclass gets editor autocomplete without writing any, and `class CreateOrder(Handler[Order])` types `self.req.data` the way `Request[Order]` already does for functions. Heaven constructs **one instance per request**, so `self` is request scoped and two requests in flight never share it; the alternative, rebinding a single instance per request, silently corrupts state the moment a handler awaits, and a concurrency test pins the behaviour. Instance attributes therefore do not persist between requests: shared state belongs on `app.keep()`. Registration resolves and checks the string eagerly, raising the new `HandlerError` at boot rather than on the first matching request when the class is not a `Handler` subclass, when the method is missing or not callable, or when the spec omits its module path or method name. Existing function handlers, whether passed directly or as a dotted string, are untouched. `heaven routes` and `heaven handlers` understand the new form: they list it as `Orders#index` and locate it at the method in your file rather than inside Heaven, and `heaven handlers /orders` prints that method's own source.
- **Feature**: **`await req.form` On Streaming Routes**. A route registered with `stream=True` can now parse `multipart/form-data` and `application/x-www-form-urlencoded` bodies incrementally: `form = await req.form` reads the body as it arrives, so field values become available while file parts never sit in memory whole. Previously `req.form` raised on a streaming route because multipart parsing needed the whole buffered body, which made streaming useless for its most common purpose, file uploads. Awaiting the form on a buffered route is harmless (it is parsed already and the await returns it unchanged), so one handler body works on both kinds of route. Reading fields on a streaming route without awaiting first raises `RuntimeError` saying to await; `req.stream()` after `await req.form` raises because the body arrives once and was consumed. Measured with `tracemalloc`: a 20MB multipart upload peaks at about 375KB of Python heap through the streamed path, a constant regardless of upload size, against 223MB through the old buffered parser.
- **Feature**: **File Parts Spill To Disk**. Streamed file parts are written to a `tempfile.SpooledTemporaryFile` as they arrive: below 256KB they stay in memory, above it they move to disk. The temp file is deleted automatically when the request is garbage collected; call `save()` to keep the upload. `File` gains `content_type`, `size`, `save(destination)` (chunked copy, never holds the file whole) and `file` (a file object positioned at the start, for handing to anything that reads files). `content` still returns the whole file as bytes, reading a spilled part back off disk, so existing code keeps working; prefer `save()` or `file` for anything large.
- **Feature**: **Form Parsing Ceilings**. Streaming made unbounded accumulation reachable again through field values, so both paths now enforce module-level ceilings, settable before serving via `heaven.form`: `FIELD_LIMIT` (1MB per non-file field value, also the cap for a streamed urlencoded body), `SPOOL_LIMIT` (256KB before a file part moves to disk), `HEADERS_LIMIT` (16KB of headers per part) and `PARTS_LIMIT` (1000 parts per form). `max_body_size` still does not apply to streaming routes; files bound their own memory by spilling to disk. Breaking a ceiling raises `ValueError`, and on a streaming route the rest of the body is drained before the error surfaces, for the same reason the `max_body_size` path drains: answering mid-upload resets the connection, and the client sees a broken pipe instead of the response. Confirmed against a real uvicorn socket, where a mid-upload 500 without the drain never arrives; the in-process test harness cannot show this.
- **Feature**: **`App(max_body_size=...)`**. Caps the request body a buffered route will accept. Past the limit the request gets `413 Payload too large` and nothing further is retained, so memory holds at the ceiling however much more is sent, rather than the size only being checked once everything is already in memory. The remainder is read and discarded rather than answered immediately, because responding mid-upload resets the connection and the client sees a broken pipe instead of the `413`; this was confirmed against a real uvicorn socket. Defaults to `None`, meaning no limit, so upgrading does not start rejecting uploads that previously worked. A proxy-level cap is still worth keeping, since it rejects oversized requests before they reach Python at all.
- **Feature**: **Streaming Uploads With `stream=True`**. `app.POST(route, handler, stream=True)`, also on `PUT` and `PATCH`, leaves the request body unread so the handler can consume it with `async for chunk in req.stream()` and write each piece straight to its destination. Peak memory is one chunk regardless of upload size: an 80MB upload costs about 36KB against roughly 160MB buffered. `req.stream()` was previously a stub returning a single raw ASGI message and is now a real async generator. On a streaming route `req.body`, `req.json`, `req.data` and `req.form` raise with an explanation, `req.stream()` raises on a buffered route for the mirror reason, and a body can only be streamed once. The flag is carried across `mount()`.
...
- **Performance**: **Buffered Multipart Parsing Drops From 11x To 2x Memory**. The buffered path previously handed the whole body to `email.parser.BytesParser`, which built the message tree and the decoded payloads alongside the raw body: 223MB peak for a 20MB upload. Both paths now share one incremental multipart parser written for this release (no new dependency), and buffered parsing feeds it in slices, so large file parts spill to their temp file instead of being copied twice more: the same 20MB upload now peaks at 40MB, the body plus one transient copy. The parser is exercised against bodies delivered 1 byte and 3 bytes at a time, and at exactly delimiter-length chunks, because a boundary split across a chunk edge is the classic bug in hand-rolled multipart parsers.
- **Performance**: **Request Bodies Buffer In Linear Time**. The body was accumulated with `body += chunk` in a receive loop, which copies everything read so far on every chunk a server delivers. At a 16KB chunk size a 20MB upload took 1.35s of pure CPU and a 40MB one took 17s, all of it blocking the event loop, so a single large upload was a denial of service regardless of memory. Chunks are now collected and joined once: the same 20MB takes 6.7ms and 40MB takes 64ms, roughly 200x and 270x faster, and doubling the size now doubles the time instead of quadrupling it.
...
- **Fix**: **Unmatched Requests No Longer Buffer Their Body**. The body was read before routing, so a 404 still paid to buffer whatever was posted to it. Routing now happens first and the body is only read once a route has been matched.
...
- **Tests**: 33 regression tests added: chunk-edge coverage at 1-byte, 3-byte and delimiter-length chunk sizes, a `tracemalloc` ceiling asserting peak memory stays under 1MB while 16MB streams through, spill-to-disk and stay-in-memory thresholds, error draining, two tests driving a real uvicorn server over a raw socket (a 20MB upload and a refused form that must answer instead of resetting), and six covering header replace semantics: case-insensitive replacement, the `Set-Cookie` exception, deletion with `None`, comma-joined list values and the `res.file()` double-header case. Fourteen more cover class handlers: async and sync methods, class-based hooks, streaming and schema validation through a class, every registration-time rejection, and a concurrency test that drives two requests into the same handler simultaneously and fails if they share an instance.
- **Tests**: 28 regression tests added in `tests/test_regressions.py` covering the correctness and hardening items above. None of those paths had coverage before.
- **Docs**: `docs/files.md` receiving-uploads section rewritten around `await req.form` (the page previously said `req.form` could not be used with streaming), plus `docs/request.md`, `docs/api.md` and `.heaven`. `docs/response.md`'s headers section now documents replace semantics; it previously described headers as append-only with no way to read, replace or delete one.
